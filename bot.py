...
import os, time, json, math, yt_dlp, telebot, sqlite3, threading, requests
...
# ======================================
# SAFE SEND WITH CHUNK UPLOAD + MULTI-UPLOADER BALANCE
# ======================================
def select_uploader():
        ...
        if size <= CHUNK_SIZE:
            ...
            with open(file_path, "rb") as f:
                ...
                for idx in range(1, total_chunks + 1):
                    ...
                    if not chunk: break
                    chunk_path = os.path.join(TEMP_DIR, f"{os.path.basename(file_path)}.part{idx}")
                    with open(chunk_path, "wb") as cf: cf.write(chunk)
                    live_progress[chat_id] = {"msg_id": None, "title": f"Uploading chunk {idx}/{total_chunks}", "percent": "", "speed": "", "eta": ""}
                    client.send_document(chat_id, chunk_path, caption=(caption if idx == 1 else None))
...
def history_cmd(m):
    ...
    if not rows: bot.send_message(m.chat.id,"No download history."); return
...
def retry_cmd(m):
    ...
    try:
        ...
        row = cursor.fetchone()
        if not row: return bot.reply_to(m,"❌ Download ID not found")
...
def callback_cmd(c):
    try:
        ...
        bot.send_message(chat_id,f"⏳ Download starting: {url} | Quality: {q}")
        queue.put((chat_id,url,q,0))
    ...
    while True:
        ...
        try:
            ...
            if retries < MAX_RETRIES:
                ...
                bot.send_message(chat,f"⚠️ Retry {retries+1}")
            else:
                failed_queue.append([chat,url,q,0])
                save_failed_queue()
                bot.send_message(chat,f"❌ Failed after {MAX_RETRIES} retries.")
    ...
    try:
        ...
        def progress(d):
            ...
            if d["status"]=="downloading" and time.time()-last_edit>1:
                ...
                try:
                    bot.edit_message_text(f"⬇️ {live_progress[chat]['title']}\n{live_progress[chat]['percent']} | {live_progress[chat]['speed']} | ETA: {live_progress[chat]['eta']}",
        ...
        ydl_opts = {
            ...
            "retries":10, "fragment_retries":10, "quiet":True,
        ...
        bot.send_message(chat,f"❌ Unexpected error: {e}")
        live_progress.pop(chat,None)
        user_threads[chat] = max(user_threads.get(chat,1)-1,0)
